...
def _convert_row_to_single_game_result(header: list[str], row: list[str]) -> GameResult:
    ...
    team1 = [player for col_name, player in row_dict.items() if col_name.startswith('Team 1 Player')]
    team2 = [player for col_name, player in row_dict.items() if col_name.startswith('Team 2 Player')]
